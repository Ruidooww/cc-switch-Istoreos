...
return view.extend({
	...
	callProxyControl: rpc.declare({
		object: 'ccswitch',
		method: 'proxy_control',
		params: [ 'action' ],
		expect: { '': {} }
	}),
	...
	handleControl: function(map, action) {
		return map.save()
			.then(L.bind(function() {
				return this.callProxyControl(action);
			}, this))
			.then(function(res) {
				if (!res || res.ok === false)
					throw new Error(res && res.error ? res.error : _('代理服务操作失败'));
	...
	renderLogs: function(logs) {
		if (!logs || !logs.length)
			return _('暂无 ccswitch-proxy 系统日志');
		return logs.join('\n');
	},
	...
	render: function(data) {
		...
		m = new form.Map('ccswitch', _('CC Switch - 路由'),
			_('运行本地代理入口，供 Claude Code 或 Codex 兼容客户端使用。'));
		...
		s = m.section(form.NamedSection, 'proxy', 'proxy', _('服务管理'));

		o = s.option(form.Button, '_start', _('启动'));
		o.inputtitle = _('启动代理');
		o.inputstyle = 'apply';
		o.onclick = L.bind(function() {
			return this.handleControl(m, 'start');
		}, this);
		...
		o.inputstyle = 'apply';
		o.onclick = L.bind(function() {
			return this.handleControl(m, 'restart');
		}, this);

		o = s.option(form.Button, '_stop', _('停止'));
		o.inputtitle = _('停止代理');
		o.inputstyle = 'reset';
		o.onclick = L.bind(function() {
			return this.handleControl(m, 'stop');
		}, this);
		...
		s = m.section(form.NamedSection, 'proxy', 'proxy', _('最近代理日志'));
		o = s.option(form.DummyValue, '_logs', _('日志'));
		o.rawhtml = true;
		o.cfgvalue = L.bind(function() {
			return E('pre', { 'style': 'white-space: pre-wrap; max-height: 260px; overflow: auto' }, this.renderLogs(status.logs));
		}, this);
