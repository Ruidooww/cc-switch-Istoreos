...
return view.extend({
	...
	callDiagnosticExport: rpc.declare({
		object: 'ccswitch',
	...
	handleDiagnosticExport: function() {
		return this.callDiagnosticExport()
			.then(function(res) {
				if (!res || res.ok === false)
					throw new Error(res && res.error ? res.error : _('导出诊断包失败'));
				downloadText(res.filename || 'ccswitch-diagnostic.txt', res.content || '');
			})
			.catch(function(err) {
				ui.addNotification(null, E('p', {}, err.message || err), 'danger');
			});
	...
	renderRows: function(entries, emptyText) {
		...
		for (var i = entries.length - 1; i >= 0; i--) {
			...
			rows.push(E('tr', { 'class': 'tr' }, [
				...
				E('td', { 'class': 'td left' }, item.curl_exit != null ? String(item.curl_exit) : ''),
				...
				E('td', { 'class': 'td left' }, item.upstream || ''),
				E('td', { 'class': 'td left' }, item.response_excerpt || '')
			]));
		}

		if (!rows.length) {
			rows.push(E('tr', { 'class': 'tr' }, [
				E('td', { 'class': 'td left', 'colspan': '15' }, emptyText)
			]));
		}
		...
		return E('div', { 'class': 'cbi-section' }, [
			...
			E('table', { 'class': 'table' }, [
				E('tr', { 'class': 'tr table-titles' }, [
					...
					E('th', { 'class': 'th left' }, _('上游')),
					E('th', { 'class': 'th left' }, _('响应摘录'))
				])
	...
	render: function(data) {
		...
		return E([], [
			...
			E('div', { 'class': 'cbi-page-actions' }, [
				...
				' ',
				E('button', { 'class': 'btn cbi-button cbi-button-neutral', 'click': L.bind(this.handleDiagnosticExport, this) }, [ _('下载诊断包') ]),
				' ',
